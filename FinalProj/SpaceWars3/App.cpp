...
App::App(const char* label, int x, int y, int w, int h) : GlutApp(label, x, y, w, h) {
	...
	//only use of TexRect
	background = new TexRect("background.bmp", 1, 1, -1, 1, 2, 2);
	...
	score = 0;
	var = -1;
	//rest are rectangle class with manual texture load
	playerChar->playerObj = playerChar->loadTexture("playership.bmp");
	...
	enemySprites.push_back(new Enemies((float)-0.8, (float)1.1, (float)0.1, (float)0.1));
	enemySprites.push_back(new Enemies((float)-0.5, (float)1.1, (float)0.1, (float)0.1));
	enemySprites.push_back(new Enemies((float)-0.2, (float)1.1, (float)0.1, (float)0.1));
	enemySprites.push_back(new Enemies((float)0.1, (float)1.1, (float)0.1, (float)0.1));
	enemySprites.push_back(new Enemies((float)0.4, (float)1.1, (float)0.1, (float)0.1));
	enemySprites.push_back(new Enemies((float)0.7, (float)1.1, (float)0.1, (float)0.1));
	...
	enemySprites.push_back(new Enemies((float)-0.95, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)-0.65, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)-0.35, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)-0.05, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)0.25, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)0.55, (float)1.3, (float)0.075, (float)0.075));
	enemySprites.push_back(new Enemies((float)0.85, (float)1.3, (float)0.075, (float)0.075));
	...
	glLoadIdentity();
	 ...
	 if (state == MENU){
		...
		for (int i = 0; i < sprites.size(); i++)
		{
			...
			sprites[i]->myObj->Rectangle::draw();
		}
		...
		glBindTexture(GL_TEXTURE_2D, playerChar->playerObj);
		playerChar->myObj->Rectangle::draw1();

		for (int i = 0; i < enemySprites.size(); i++){
			if(!checkHit[i]){

				glBindTexture(GL_TEXTURE_2D, enemySprites[i]->sprite);
				enemySprites[i]->myObj->Rectangle::draw1();
		...
		print(-0.25, 0.5, 0.0, "Controls");
		print(-0.25, 0.3, 0.0, "Up = W");
		print(-0.25, 0.2, 0.0, "Left = A");
		print(-0.25, 0.1, 0.0, "Down = S");
		print(-0.25, 0.0, 0.0, "Right = D");
		print(-0.25, -0.1, 0.0, "Shoot = SpaceBar");
		print(-0.25, -0.2, 0.0, "Spepcial Shot = V");
		print(-0.25, -0.3, 0.0, "Menu = Esc");
		print(-0.25, -0.4, 0.0, "Restart Game = R");
...
void App::keyPress(unsigned char key) {
	...
	if (paused == 1 && state == PLAYING) {
		...
		if (key == 32) {
			...
			missleX = playerChar->myObj->getX() + (float)0.04;
			missleY = playerChar->myObj->getY() + (float)0.05;
			...
			missleX = playerChar->myObj->getX() + (float)0.04;
			missleY = playerChar->myObj->getY() + (float)0.05;
			missleX2 = playerChar->myObj->getX() + (float)0.04;
			missleY2 = playerChar->myObj->getY() + (float)0.05;
			missleX1 = playerChar->myObj->getX() + (float)0.04;
			missleY1 = playerChar->myObj->getY() + (float)0.05;
			r1 = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			r2 = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			r3 = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			m1.push_back(new Rectangle(missleX, missleY + (float)0.05, (float)0.02, (float)0.03, r1, r2, r3));
			...
			if(up){
				...
				playerChar->myObj->setY(playerChar->myObj->getY() + 0.05);

			}else{

				playerChar->myObj->setY(playerChar->myObj->getY());
			...
			if(left){
				...
				playerChar->myObj->setX(playerChar->myObj->getX() - 0.05);

			}else{

				playerChar->myObj->setX(playerChar->myObj->getX());
...
void App::idle() {
	...
	if (gameLoop == 1 && state == PLAYING) {
		...
		if(paused){
			...
			float x = playerChar->myObj->getX();
			float y = playerChar->myObj->getY();
			...
			for (int i = 0; i < enemySprites.size(); i++){
				...
				for (int j = 0; j < m1.size(); j++){
					...
					if (enemySprites[i]->myObj->contains(m1[j]->getX(), m1[j]->getY()) && !checkHit[i]){
			...
			for (int i = 0; i < enemySprites.size(); i++){
				...
				for (int j = 0; j < m2.size(); j++){
					...
					if (enemySprites[i]->myObj->contains(m2[j]->getX(), m2[j]->getY()) && !checkHit[i]){
			...
			for (int i = 0; i < enemySprites.size(); i++){
				...
				for (int j = 0; j < m3.size(); j++){
					...
					if (enemySprites[i]->myObj->contains(m3[j]->getX(), m3[j]->getY()) && !checkHit[i]){
			...
			for (int i = enemySprites.size() - 1; i >= 0; i--){
				...
				if (enemySprites[i]->myObj->getY() > -1 && paused && !checkHit[i]){

					enemySprites[i]->myObj->setY(enemySprites[i]->myObj->getY() - 0.0009);

				}else if(enemySprites[i]->myObj->getY() > -1 && paused && checkHit[i]){

					enemySprites[i]->myObj->setY(enemySprites[i]->myObj->getY());
...
void App::resetGame(int gameStateNum){
	...
	spritesKilled = 0;
	score = 0;
	playerChar->myObj->setX(0.0);
	playerChar->myObj->setY(-0.7);
	enemySprites.clear();
	m1.clear();
	m2.clear();
	m3.clear();
